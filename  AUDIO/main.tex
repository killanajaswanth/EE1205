...
\author{JASWANTH KILLANA\\EE23BTECH11033}
\date{}
...
\begin{align}
    \omega_{p1} &= 2\pi\frac{F_{p1}}{F_s} = 0.191 \pi\\
    \omega_{p2} &= 2\pi\frac{F_{p2}}{F_s}  =0.241 \pi
\end{align}
...
\begin{align}
    F_{s1} &= 4.6-0.3 = 4.1 \text{KHz}\\
    F	_{s2} &= 5.8 +0.3 = 6.1  \text{KHz}
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.170 \pi\\
...
$\Omega_{p1} = 0.309$, $\Omega_{p2} = 0.398$ and $\Omega_{s1} = 0.274$, $\Omega_{s2} = 0.422$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.351$ and $B = \Omega_{p2} - \Omega_{p1} = 0.089$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.973\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.461
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.461.
\end{align}
...
we get $N\geq 4$ and $0.323 \leq \epsilon \leq 0.61$\\
\newpage
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/killanajaswanth/EE1205/tree/main/Audio_%20Filter/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/plot1.png}
\caption{The Analog Low-Pass Frequency Response for $0.323  \leq \epsilon \leq 0.61$}
...
\begin{lstlisting}
https://github.com/killanajaswanth/EE1205/blob/main/Filter_Design/codes/sk_gen.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/killanajaswanth/EE1205/blob/main/Filter_Design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
Substituting respective values in \eqref{eq:Gain_eq_LP} we get $G_{LP}=0.507$
\begin{align}
     H_{a,LP}(s_L) &= \frac{0.507}{\brak{s_L-s_5}\brak{s_L-s_6}\brak{s_L-s_7}\brak{s_L-s_8}}\\
     &= \frac{0.507}{s_{L}^4 + 1.423s_{L}^3 +1.821s_{L}^2 + 1.099s_{L} +0.45003}\label{eq:design}
\end{align}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{6.57\times 10^{-5}s^4}{s^8 + 0.154s^7 + 0.182s^6 + 0.1710s^5 + 1.07s^4 + 0.790s^3 + 0.249s^2 + 0.0107s + 0.027} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{align}
     \frac{\sin(\omega_{p2} n)}{n\pi} -\frac{\sin\brak{\omega_{p1}n}}{n\pi} &= 2\cos{\brak{\frac{\omega_{p2}n+\omega_{p1}n}{2}}}\sin{\brak{\frac{\omega_{p2}n-\omega_{p1}n}{2}}}\\
            &= \frac{2\cos{\brak{0.216n\pi}}\sin{\brak{0.025n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{0.216n\pi}}\sin{\brak{0.025n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
\end{cases} \label{eq:h(n)desired_for_LPF}
\end{align}
